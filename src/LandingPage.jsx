...
export default function LandingPage({ onNavigate }) {
  return (
    <div style={{ minHeight: "100vh", background: K.bg, color: K.ink, fontFamily: FM, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
      ...
      <div style={{ display: "flex", gap: 20, flexWrap: "wrap", justifyContent: "center", padding: "0 20px", maxWidth: 700 }}>
        {/* Rankine Cycle Card */}
        <button
          onClick={() => onNavigate("rankine")}
          style={{
            background: "#fff", border: `1.5px solid ${K.border}`, padding: "28px 32px", cursor: "pointer",
            width: 280, textAlign: "left", transition: "all 0.2s",
        ...
        >
          <div style={{ fontSize: 9, color: K.inkLight, letterSpacing: 2, marginBottom: 6, textTransform: "uppercase" }}>Power Generation</div>
          <div style={{ fontSize: 20, fontFamily: FD, color: K.ink, marginBottom: 8, lineHeight: 1.2 }}>
            Rankine <span style={{ color: K.accent, fontStyle: "italic" }}>Cycle</span>
          </div>
          <div style={{ fontSize: 10, color: K.inkMed, lineHeight: 1.7 }}>
          ...
          <div style={{ marginTop: 14, display: "flex", gap: 8 }}>
            {["Turbine", "Boiler", "Pump", "Condenser"].map((c, i) => (
              <span key={i} style={{ fontSize: 8, padding: "2px 6px", background: K.cardAlt, border: `1px solid ${K.border}`, color: K.inkLight }}>{c}</span>
            ))}
          </div>
        </button>
        ...
        <button
          ...
          onMouseLeave={e => { e.currentTarget.style.borderColor = K.border; e.currentTarget.style.boxShadow = "0 2px 12px rgba(0,0,0,0.06)"; }}
        >
          <div style={{ fontSize: 9, color: K.inkLight, letterSpacing: 2, marginBottom: 6, textTransform: "uppercase" }}>Cooling & Refrigeration</div>
          <div style={{ fontSize: 20, fontFamily: FD, color: K.ink, marginBottom: 8, lineHeight: 1.2 }}>
      ...
      <div style={{ textAlign: "center", padding: "48px 12px 28px", fontSize: 9, color: K.inkLight, fontFamily: FM, fontStyle: "italic", letterSpacing: 1 }}>
